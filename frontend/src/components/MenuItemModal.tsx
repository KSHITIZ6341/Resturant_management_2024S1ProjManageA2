...
interface MenuItemModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (item: MenuItemRecord) => void;
  item: MenuItemRecord | null;
}

const MenuItemModal = ({ isOpen, onClose, onSave, item }: MenuItemModalProps) => {
  const [formData, setFormData] = useState<MenuItemRecord>({ category: 'ENTREE', name: '' });
  ...
  const handleCategoryChange = (_event: React.MouseEvent<HTMLElement>, newCategory: string | null) => {
    if (newCategory !== null) {
      setFormData((prev) => ({ ...prev, category: newCategory as MenuItemRecord['category'] }));
    }
  };
