...
class PrintableOrder extends React.Component {
  render() {
    const { formData, customers } = this.props;
    const customerName = customers.find(c => c.id === parseInt(formData.customer_id, 10))?.name || 'N/A';
    const orderedItems = Object.entries(formData.order_data).filter(([, v]) => v.quantity > 0);
