...
const PROTECTED_DEFAULT_IDS = new Set(['admin', 'manager']);
...
function normaliseUsers(users: UserAccount[]): UserAccount[] {
  const dedupedByName = new Map<string, UserAccount>();
  users.forEach((user) => {
    const nameKey = user.name.toLowerCase();
    if (!dedupedByName.has(nameKey)) {
      dedupedByName.set(nameKey, user);
    }
  });
  ...
  dedupedByName.forEach((user) => {
    if (PROTECTED_DEFAULT_IDS.has(user.id)) {
      return;
    }
    byId.set(user.id, { ...user, name: sanitiseName(user.name) });
  });
